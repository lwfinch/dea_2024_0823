            ...
            if self.args.sample_last_num_steps >= 10:
                ...
                if np.sum(dones) > 0:
                    if np.sum(faileds) > 0:
                        for info in infos:
                            if info["out_of_road"]:
                                print("sample_steps:", sample_steps, " / 200000   ", float(sample_steps/200000*100), "%")
                                sample_steps += np.sum(
                                    np.clip(steps[faileds], 0, self.args.sample_last_num_steps))
                                tuple(
                                    map(insert_memory, tmp_memorys[faileds]))
                        '''for info in infos:
                            if info["crash_vehicle"] and num_cv < 140000:
                                num_cv += 20
                                print("sample_steps:", sample_steps, " / 300000   ", float(sample_steps/300000*100), "%")
                                sample_steps += np.sum(
                                    np.clip(steps[faileds], 0, self.args.sample_last_num_steps))
                                tuple(
                                    map(insert_memory, tmp_memorys[faileds]))
                            elif info["out_of_road"] and num_oor < 60000:
                                num_oor += 20
                                print("sample_steps:", sample_steps, " / 300000   ", float(sample_steps/300000*100), "%")
                                sample_steps += np.sum(
                                    np.clip(steps[faileds], 0, self.args.sample_last_num_steps))
